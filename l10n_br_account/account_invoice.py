...
class account_invoice_line(orm.Model):
    ...
    def _validate_taxes(self, cr, uid, values, context=None):
        """Verifica se o valor dos campos dos impostos estão sincronizados
           com os impostos do OpenERP. Por padrão, não faz nada. Deve ser 
           extendido pelo módulo implementando a lógica de validação.
        """
        
        return {}
    
    def product_id_change(self, cr, uid, ids, product, uom, qty=0, name='',
                          _type='out_invoice', partner_id=False,
        ...
        result = super(account_invoice_line, self).product_id_change(
            cr, uid, ids, product, uom, qty, name, _type, partner_id,
            fposition_id, price_unit, currency_id, context, company_id)
        
        if product:
            p = self.pool.get('product.product').browse(cr, uid, product, context)
            if p.property_fiscal_classification:
                result['value']['fiscal_classification_id'] = p.property_fiscal_classification.id
